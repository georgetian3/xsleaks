...
假设资源的 URL 是 \code{target.com/resource}。只有 \code{audio} 以及 \code{video} 元素在 URL 中加入 \code{size} 参数时，即 \code{target.com/resource&size=}比如 浏览器才会
...
\code{performance.getEntries()} 的稳定性也较差。
